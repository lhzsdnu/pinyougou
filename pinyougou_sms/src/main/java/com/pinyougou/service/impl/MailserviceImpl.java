...
public class MailserviceImpl implements MailService {
    ...
    public void sendMessageMail(Object params, String title, String templateName) {
        ...
        try {
            ...
            helper.setTo(InternetAddress.parse(from));//发送给谁
    ...
    @Override
    public void sendMessageMail(Object params, String title, String templateName,String toUser) {
        try {

            MimeMessage mimeMessage = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true);
            helper.setFrom(from);//发送者
            helper.setTo(InternetAddress.parse(toUser));//发送给谁
            helper.setSubject("【" + title + "-" + LocalDate.now() + " " + LocalTime.now().withNano(0) + "】");//邮件标题
